...
/**
 * Represents the scopes that this project will use regarding Google's API.
 * As we want to modify spreadsheets, we'll use that scope.
 */
...
/**
 * Initializes a new auth instance to initialize a new Google client.
 * @param  dir The root directory that holds the credentials and tokens file.
 * @return     An authorized client with Google's API.
 */
export async function authorize(dir: string = paths.config): Promise<OAuth2Client> {
